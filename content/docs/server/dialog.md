...
{{< alert context="info" text="Built. The knowledge model, the conversation wire, history phrasings, small talk and the rumour ledger run on the server, and the client has a conversation panel. The panel is the one part written without being run — there is no Godot on the machine it was written on — so treat [Building the conversation UI](#building-the-conversation-ui) as a checklist for the first person to open the project rather than as finished work." />}}
...
and the `DialogType` enum called a choice dialog _"planned but out of scope"_. Conversation took that
...
`Households.one` is built for exactly this: _"household 400 is the same household whether or not
households 0 to 399 were expanded first"_. The house RNG convention applies unchanged — **keyed
...
everything here: a root of a handful of options, and a leaf that says one thing and offers _back_.
...
A merchant always has _buy_ and _sell_. This is not a special case bolted onto a random system, it
...
The action tier ships **stubbed**: a merchant's _show me your wares_ appears and answers "not yet".
...
inherited story, within twenty-five kilometres, _or_ important enough that everybody knows it.
...
probability threshold: a per-person coin flip at _p = 1/n_ is a binomial draw, so it would sometimes
...
**The priest is only _likely_ to be the one.** `weight` biases the ranking; it does not filter it.
...
Note also what the bottom row is _not_: a trade's interest never admits a memory to the pool, it
...
A given farmer therefore has _their_ topic, about _their_ fields, in this valley, this season — and
...
player clicks _and_ the answer. It is worth writing that number down, because a flavour pool with no
...
normalising: a shared event row plus a join table would be one truth about _where_ news reached, and
...
_driven_ by something — it shipped reachable only from a GM command at first, which is the kind of
...
grandmother. And the topic ids have to be disjoint from the chronicle's _and_ stay inside their
...
"Karth" is Karth in every locale. Only the _connective_ words need tokens, and separating the two is
...
`Memory` deliberately declines to make on the caller's behalf: _"how recent something feels is a
decision for whoever is speaking — a grandmother and a chronicler place the same year
differently."_ A speaker's own sense of time is a property of the speaker.
...
It is wrong for a line whose key is _computed_. A history line's key derives from an `EventKind`
...
| ------------------------ | ------------------------- | ------------------------------------------------------------------------------------------------------------ |
| `InteractCMSG`           | new                       | `entity_id`. "Talk to that" — the _server_ decides what interacting with an entity means                     |
...
today holds exactly one action — grows a _Talk to_ entry. And the message dialog grows buttons,
which its own source comment already anticipates: _"Player-choice dialogs will add buttons here and
send the answer back… the queue and text resolution above and below this node do not change."_
...
1. The player asks the **innkeeper**, because that is what everyone does. She has the burial: it is
   ...
   fourth.
2. So the player goes asking who remembers anything _more_ about that night. Nobody does — not the
...
out of her rumour list, because that half of it _is_ stored.
...
1. **Every `EventKind` is voiced** in `townsfolk/dialogue.yml`. Without it an NPC can _know_
   something it cannot _say_. This is the `LIGHTHOUSE_LIT` mistake in a new place: a thing that
...
The panel exists: `conversation_presenter.gd` turns a `ConversationSMSG` into a `DialogContent` and
`MessageDialog` renders its options as buttons. **It has never been run.** There is no Godot on the
machine it was written on and the client targets net8.0 against an SDK 7, so it is neither compiled
nor executed — the message property names were checked against the C# wrappers and the structure
against the surrounding files, and that is the whole of the verification.

So this section is now a checklist for whoever opens the project first, not a plan.

**Check the layout before anything else.** `MessageDialog.tscn` gained a `Content: VBoxContainer`
wrapping `%Body` and a new `%Choices`, written by hand rather than by the editor. `%Body` is
`unique_name_in_owner`, so the script's lookup survives the move — but the anchors and sizing are the
part most likely to want an eye.

**The shape it was built to, which should not need changing.** `MessageDialog` grew rather than a
second scene being added, because `DialogManager` holds a **single presenter slot** and two presenters
would mean two copies of the `confirmed`/`canceled`/`_current`-guard dance. A conversation is a
`DialogContent` whose `options` are non-empty; the queue and the text resolution are the ones dialogs
have always used.

**`_on_closed` is idempotent through its `_current == null` guard, and must stay so.** Closing raises
both `confirmed` and `canceled`, and a double call silently skips the next queued dialog. Choosing an
option answers _first_ and closes second, because closing clears `_current` and with it the callable
the answer needs.

**What a choice means stays out of the window.** `DialogContent` carries a `Callable`; the window
renders options and reports which was picked, and knows nothing about conversations. That is what
keeps the next thing with buttons from having to be a conversation.

**`DialogText.resolve_line` already serves both** speech and option labels — translate the key, then
treat the result as a template. No third resolver is needed, and a raw server string must never reach
a label.
